...
Для анонимного голосования и работы администраторских инструментов установите следующие правила в вашей Realtime Database:
```json
{
  "rules": {
    "sessions": {
      ".read": true,
      "$sessionId": {
        // Для админ-инструментов: разрешить запись в config и photos.
        // ВНИМАНИЕ: Это небезопасно для публичных приложений.
        // Для защиты используйте правила аутентификации Firebase (например, "auth != null && auth.uid === 'YOUR_ADMIN_UID'").
        "config": { ".write": true },
        "photos": { ".write": true },
        
        // Правила для голосования остаются без изменений
        "votes": {
          "$photo_id": {
