    ...
    <div id={`photo-card-${photo.id}`} className={containerClasses} style={{opacity: isDimmed ? 0.5 : 1}}>
      <div onClick={handleCardClick} className={`${!isReadOnly ? 'cursor-pointer' : ''} relative z-[1]`}>
        <img
          src={photo.url}
          alt={`Фото ${photo.id}`}
          className={`w-full h-auto object-cover transition-transform duration-300 ${!isReadOnly ? 'group-hover:scale-105' : ''} ${layoutMode === 'grid' ? aspectRatioClass : ''} ${isGrayscale ? 'grayscale brightness-75' : ''}`}
          loading="lazy"
        />
      </div>
