...
export const PhotoCard: React.FC<PhotoCardProps> = ({ photo, onRate, onImageClick, displayVotes, layoutMode, gridAspectRatio }) => {
  ...
  const containerClasses = `group relative overflow-hidden rounded-lg shadow-lg bg-gray-800 border border-gray-700/50 transition-all duration-300 hover:shadow-indigo-500/30 hover:border-indigo-500/50 ${voteRingClass} ${layoutMode === 'original' ? 'break-inside-avoid' : ''}`;
    ...
    <div id={`photo-card-${photo.id}`} className={containerClasses}>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2 flex justify-between items-center">
        <div className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300" onClick={e => e.stopPropagation()}>
           <RatingControls photo={photo} onRate={onRate} size="small"/>
        </div>
        {displayVotes && (
       ...
       <div className="absolute top-2 left-2 bg-black/50 text-white text-xs font-mono px-2 py-1 rounded pointer-events-none opacity-70 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        {photo.id}
      </div>
    </div>
  );
