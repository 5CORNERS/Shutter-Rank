    ...
    <div id={`photo-card-${photo.id}`} className={containerClasses}>
      <div onClick={handleCardClick} className={`${!isReadOnly ? 'cursor-pointer' : ''} relative z-[1]`}>
        <img
          src={photo.url}
          alt={`Фото ${photo.id}`}
          className={`w-full h-auto object-cover transition-transform duration-300 ${!isReadOnly ? 'group-hover:scale-105' : ''} ${layoutMode === 'grid' ? aspectRatioClass : ''}`}
          loading="lazy"
        />
      </div>
      ...
      <div className="absolute top-2 left-2 z-[3] flex items-center gap-1.5">
        {!isOutOfComp && !isReadOnly && showVisibilityToggle && (
          <button
            ...
            className="p-1.5 rounded-full bg-black/50 text-white/80 hover:bg-black/70 hover:text-white opacity-70 md:opacity-0 group-hover:opacity-100 transition-all duration-300 disabled:opacity-30 disabled:hover:bg-black/50 disabled:cursor-not-allowed"
