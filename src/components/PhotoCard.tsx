...
export const PhotoCard: React.FC<PhotoCardProps> = ({ photo, onRate, onImageClick, onToggleFlag, displayVotes, layoutMode, gridAspectRatio }) => {
  ...
  const containerClasses = `group relative overflow-hidden rounded-lg shadow-lg bg-gray-800 border transition-all duration-300 hover:shadow-indigo-500/30 ${voteRingClass} ${layoutMode === 'original' ? 'break-inside-avoid' : ''} ${competitionClass}`;

  return (
    <div id={`photo-card-${photo.id}`} className={containerClasses} data-photo-id={photo.id}>
      <img
        src={photo.url}
        alt={`Фото ${photo.id}`}
